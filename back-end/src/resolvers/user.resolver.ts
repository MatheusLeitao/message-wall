...
export default class UserResolver {
    ...
    public async createOrLoginUser(@Args('data') input: UserInput): Promise<User> {
        ...
        let user = await this.repoService.userRepo.findOne({ where: { email: input.email.toLocaleLowerCase().trim()}})

        if(!user) {
            await this.repoService.userRepo.create({ email: input.email.toLocaleLowerCase().trim() });
        }
